...
import { trendInlineActions, trendMeter, trendModuleHead, trendRuleEditor, trendToggleField } from './today-trend-ui.js';
...
function eventCard(event, archived, attrs, actionsVisible) {
    ...
    return `<article class="pm-today-trend-event-card${archived ? ' is-archived' : ''}" data-event-type="${escapeAttr(event.type)}"><span class="pm-today-trend-event-marker" aria-hidden="true">${eventIcon(event)}</span><div class="pm-today-trend-event-body"><header><div class="pm-today-trend-event-heading"><b>${text(event.title)}</b><span class="pm-today-trend-event-tags">${badge(event)}${pill(event, archived, state)}</span></div>${trendInlineActions({ visible: actionsVisible, actions })}</header><dl class="pm-today-trend-event-facts"><div><dt>起因</dt><dd>${text(event.origin)}</dd></div><div><dt>主体</dt><dd>${text(event.participants.join('、') || '未记录')}</dd></div></dl>${history}${archived ? `<div class="pm-today-trend-event-latest"><strong>最终结果</strong><span>${text(event.finalResult)}</span></div>` : ''}</div></article>`;
}
export function renderTodayTrendDynamicsView({ scope, preset = null, editingEventId = null, editingRule = null, ruleDraft = null, mode = 'content', dynamicsTab = 'active', menuOpenId = null, generationAvailable = false, generationBusy = false } = {}) {
    ...
    const active = scope.dynamics.active.map(event => eventCard(event, false, attrs, actionsVisible)).join('') || '<p class="pm-today-trend-empty">暂无正在追踪的动态。</p>';
    const archived = scope.dynamics.archived.map(event => eventCard(event, true, attrs, actionsVisible)).join('') || '<p class="pm-today-trend-empty">暂无已完结动态。</p>';
    ...
    return `<section class="pm-today-trend-view pm-today-trend-dynamics">${trendModuleHead({ title, eyebrow: 'EVENT TRACKER', metaHtml: trackerMeta, menuId: 'dynamics-module', menuOpenId, actions: [icon('today-trend-advance-all-events', REFRESH_ICON_SVG, '重新生成事件追踪', attrs), icon('today-trend-edit-dynamics-rule', BOOK_ICON_SVG, '编辑事件追踪 Prompt'), icon('today-trend-open-dynamics-settings', SETTINGS_ICON_SVG, '事件追踪设置')], adornment: '<span class="pm-today-trend-dynamics-target" aria-hidden="true"><svg viewBox="-58 -58 116 116" fill="none" stroke="currentColor"><defs><radialGradient id="pm-today-trend-dynamics-target-glow"><stop stop-color="currentColor" stop-opacity=".22"/><stop offset=".46" stop-color="currentColor" stop-opacity=".11"/><stop offset=".8" stop-color="currentColor" stop-opacity=".025"/><stop offset="1" stop-color="currentColor" stop-opacity="0"/></radialGradient></defs><circle r="58" fill="url(#pm-today-trend-dynamics-target-glow)" stroke="none"/><circle r="48" stroke-width=".5" stroke-dasharray="2 3" opacity=".16"/><circle r="34" stroke-width=".5" stroke-dasharray="12 4" opacity=".14"/><circle r="20" stroke-width=".75" opacity=".2"/><path d="M-58 0H58M0-58V58" stroke-width=".5" opacity=".12"/><path d="M-14-43A45 45 0 0 1 32-31M42 14A45 45 0 0 1 13 43M-38 24A45 45 0 0 1-43-12" stroke-width="1" opacity=".21"/><circle r="3" fill="currentColor" stroke="none" opacity=".28"/></svg></span>' })}${editor}${tabSwitch}${activePanel}${archivedPanel}${generationBusy ? '<span class="pm-today-trend-progress">正在生成…</span>' : ''}</section>`;
}
