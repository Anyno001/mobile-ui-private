...
export function renderTodayTrendDynamicsView({ scope, preset = null, editingEventId = null, editingRule = null, ruleDraft = null, mode = 'content', dynamicsTab = 'active', menuOpenId = null, generationAvailable = false, generationBusy = false, floorStatus = '' } = {}) {
    ...
    return `<section class="pm-today-trend-view pm-today-trend-dynamics">${trendModuleHead({ title, metaHtml: trackerMeta, asideHtml: floorStatus, menuId: 'dynamics-module', menuOpenId, exposedActionCount: 2, itemActions: true, actions: [icon('today-trend-advance-all-events', REFRESH_ICON_SVG, '重新生成事件追踪', attrs), icon('today-trend-edit-dynamics-rule', BOOK_ICON_SVG, '编辑事件追踪提示词'), icon('today-trend-open-dynamics-settings', SETTINGS_ICON_SVG, '事件追踪设置')] })}<div class="pm-today-trend-module-body">${tabSwitch}${activePanel}${archivedPanel}</div></section>`;
}
