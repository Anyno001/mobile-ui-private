...
export function trendModuleHead({ title, menuId, menuOpenId, actions = [], meta = '', metaHtml = '', adornment = '', asideHtml = '', exposedActionCount = 0, itemActions = false }) {
    ...
    return `<header class="pm-today-trend-module-head is-expanded"><div>${renderedMeta}<h2>${escapeHtml(title)}${adornment}</h2></div><span class="pm-today-trend-head-tools"></span><span class="pm-today-trend-module-actions">${firstAction}${quickActions.join('')}${menu}${asideHtml}</span></header>`;
}
