...
export function renderTodayTrendWorldView({ scope, preset = null, mode = 'content', editingWorldItemId = null, editingRule = null, ruleDraft = null, menuOpenId = null, generationAvailable = false, generationBusy = false } = {}) {
    ...
    const rows = items.map((item, index) => {
        ...
        if (index === 0) return `<article class="pm-today-trend-world-hero" data-world-item-id="${escapeAttr(item.id)}"><span class="pm-today-trend-world-dotfield is-hero" aria-hidden="true"></span><div class="pm-today-trend-world-panel"><header class="pm-today-trend-world-item-head"><b>${escapeHtml(item.name)}</b>${itemActions(item, generateAttrs, actionsVisible)}</header>${body}</div></article>`;
        const side = index % 2 ? 'is-left' : 'is-right';
        return `<article class="pm-today-trend-world-brief ${side}" data-world-item-id="${escapeAttr(item.id)}">${side === 'is-right' ? '<span class="pm-today-trend-world-left-ornament" aria-hidden="true"></span>' : ''}<span class="pm-today-trend-world-dotfield" aria-hidden="true"></span><div class="pm-today-trend-world-panel"><span class="pm-today-trend-world-ornament" aria-hidden="true"></span><header class="pm-today-trend-world-item-head"><b>${escapeHtml(item.name)}</b>${itemActions(item, generateAttrs, actionsVisible)}</header>${body}</div></article>`;
    }).join('');
    const editor = editingRule === 'world' ? trendRuleEditor({ rule: editingRule, value: ruleDraft ?? preset?.moduleRules?.world ?? '' }) : '';
    return `<section class="pm-today-trend-view pm-today-trend-world">${trendModuleHead({ title: '世界态势', menuId: 'world-module', menuOpenId, adornment: '<span class="pm-today-trend-world-title-dotfield" aria-hidden="true"></span>', actions: [{ action: 'today-trend-generate-world', icon: REFRESH_ICON_SVG, label: '重新生成世界态势', attrs: generateAttrs }, { action: 'today-trend-edit-world-rule', icon: BOOK_ICON_SVG, label: '编辑世界态势 Prompt' }] })}${editor}<span class="pm-today-trend-world-starfield" aria-hidden="true"></span>${rows || '<p class="pm-today-trend-empty">尚未生成世界态势。</p>'}${generationBusy ? '<span class="pm-today-trend-progress">正在生成…</span>' : ''}</section>`;
}
