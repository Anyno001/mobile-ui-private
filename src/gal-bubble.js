...
export const GAL_BUBBLE_SCRIPT_NAME = '天音小笺-GAL气泡';
...
export function parseGalBubbleMessages(raw) {
    ...
    while ((match = GAL_BUBBLE_MESSAGE_PATTERN.exec(source)) !== null) {
        ...
        if (name && text) messages.push({ side: match[1].toLowerCase(), name, text });
    }
...
export function uninstallGalBubble(context) {
    const list = getRegexList(context);
    const matches = list.filter(isOwnedScript);
    if (matches.length > 1) throw new Error('发现多条同 ID 的天音小笺正则，已停止修改，请先在酒馆正则面板清理重复项');
