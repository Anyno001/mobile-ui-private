...
import { outfitForDate } from './calendar-outfit-model.js';
...
import {
    BACK_ICON_SVG, CALENDAR_ICON_SVG, CHEVRON_DOWN_ICON_SVG, FORWARD_ICON_SVG, MOON_ICON_SVG,
    HOME_ICON_SVG, OUTFIT_ICON_SVG, RECIPE_ICON_SVG, REFRESH_ICON_SVG, SPARKLES_ICON_SVG, WEATHER_ICON_SVG,
} from './icons.js';
    ...
    const summary = viewMode === 'weather'
        ...
        : viewMode === 'cycle'
            ...
            : viewMode === 'recipe'
                ...
                : viewMode === 'outfit'
                    ? outfit?.text || lunarLabel(parsed)
                    : holidays[0]?.name || occasions[0]?.title || events[0]?.title || lunarLabel(parsed);
    return { parsed, events, occasions, holidays, weather, cycle, recipe, outfit, summary,
        hasSchedule: events.length > 0 || occasions.length > 0, hasRecipe: Boolean(firstMeal), hasOutfit: Boolean(outfit?.text) };
}
...
export function renderCalendarPageHtml(
    ...
    view = {}, recipeScope = {}, outfitProfile = {},
) {
    const today = calendarReferenceDate(scope);
    const viewMode = ['schedule', 'weather', 'cycle', 'recipe', 'outfit'].includes(view.viewMode) ? view.viewMode : 'schedule';
    ...
    const days = monthCells.map(cell => {
        ...
        else if (viewMode === 'outfit' && meta.hasOutfit) classes.push('has-outfit');
    ...
    const detailRegenerating = viewMode === 'recipe'
        ...
        : viewMode === 'outfit'
            ? view.outfitGenerating === true && view.outfitGenerationTask?.mode === 'outfit-regenerate'
    ...
    const selectedDetail = renderSelectedDateDetail(
        ...
        view.detailEditing === true, detailRegenerating, outfitProfile,
    );
    const headerAction = viewMode === 'weather' ? 'calendar-weather-refresh'
        : viewMode === 'schedule' ? 'calendar-generate'
            : viewMode === 'recipe' ? 'calendar-recipe-generate'
                : viewMode === 'outfit' ? 'calendar-outfit-generate' : '';
    const recipeWindow = calendarWindowDescription(today, 7);
    const headerActionLabel = viewMode === 'weather' ? '刷新天气'
        : viewMode === 'recipe' ? `AI 生成${recipeWindow.label}菜谱`
            : viewMode === 'outfit' ? `AI 生成${recipeWindow.label} OOTD` : calendarGenerationCopy(today).actionLabel;
    ...
    const management = renderCalendarManagement({
        scope, holidayCache, weatherStore, cycleScope, recipeScope, outfitProfile, weatherResults, viewMode,
        holidayAvailable, holidayRange, editorKind: view.editorKind, cycleSubjects: view.cycleSubjects,
        selectedCycleSubject: view.cycleSubject, outfitSubjects: view.outfitSubjects,
        selectedOutfitSubject: view.outfitSubject, managementOpen: view.managementOpenByMode?.[viewMode],
    });
    ...
    const headerBusy = viewMode === 'weather' ? view.weatherRefreshing === true : viewMode === 'recipe'
        ? view.recipeGenerating === true : viewMode === 'outfit' ? view.outfitGenerating === true : viewMode === 'schedule' && view.generating === true;
