...
1. 严禁输出对界面、系统、对话本身的总结或描述性文字
1. 严禁输出类似”现在应该…”、“我已经…”、“看起来…“这类叙述性句子
1. 特殊格式必须在同一行内完整写出且闭合：(转账+金额) (收款+金额) (退还+金额) (图片+描述) (语音+内容)。注意：退还指拒绝聊天对象转账。
1. 特殊格式括号内严禁换行、编号（1. 2. 3.）、列表
1. 每条消息内的 / 只用于分隔同一角色的多条短信
1. 每个角色根据自己的人设和当前剧情主动决定发言条数，0-8句，可穿插发言，不必所有人都说话
1. 严禁英文格式 (Voice+/Image+/Transfer+/Refund+)
1. 完全沉浸于角色设定，褪去AI客观语气。根据用户引导自然推进剧情，在用户明确发起成人或极端互动前，保持符合日常社交尺度的全年龄对话风格。
...
1. 第一点)
   小红：界面现在应该正常了…`; injectedInstruction = `${groupRules}
...
${userName}：${userMsg}`; systemPrompt = [ `你同时扮演 ${memberList} 在群聊「${groupName}」中与用户 ${userName} 对话。`, `【用户信息】\n${userBlock}`, cardDesc ? `【角色设定】\n${cardDesc}`: '', cardPersonality ?`【性格】\n${cardPersonality}`: '', cardScenario ?`【场景】\n${cardScenario}`: '', worldBookText ?`【世界书】\n${worldBookText}`: '', mainChatText ?`【主线最近对话】\n${mainChatText}`: '', '',`输出格式：角色名：消息 / 消息（每个角色0-8句，根据人设和剧情决定是否发言及发言数量）`, `角色名后只跟该角色的话，严禁 “(角色名：xxx)” 这种嵌套。`, `角色可穿插发言，不必所有人都说话。`, '特殊格式（必须中文且单行闭合）：(转账+金额) (收款+金额) (退还+金额) (图片+描述) (语音+内容)。注意：退还指拒绝聊天对象转账。', '禁止任何标签格式旁白选项状态栏。', ].filter(Boolean).join('\n\n'); } else { const contextBlockMain = [ cardScenario ? `【场景参考】\n${cardScenario}`: '', cardMesExample ?`【对话示例】\n${cardMesExample}`: '', ].filter(Boolean).join('\n\n'); injectedInstruction =`
[短信模式指令——最高优先级]
当前角色：${currentPersona}
...
${userName}：${userMsg}
${currentPersona}：`; systemPrompt = [ `你正在扮演”${currentPersona}“通过手机短信与用户 ${userName} 聊天。`, `【用户信息】\n${userBlock}`, cardDesc ? `【角色设定】\n${cardDesc}`: '', cardPersonality ?`【性格】\n${cardPersonality}`: '', cardScenario ?`【场景】\n${cardScenario}`: '', cardFirstMes ?`【开场白参考】\n${cardFirstMes}`: '', cardMesExample ?`【对话示例】\n${cardMesExample}`: '', worldBookText ?`【世界书】\n${worldBookText}`: '', mainChatText ?`【主线最近对话】\n${mainChatText}` : ‘’,
‘’,
