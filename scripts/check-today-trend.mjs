...
assert.match(todayTrendStyle, /pm-today-trend-event-facts\{[^}]*gap:var\(--pm-space-0-5\)[^}]*margin-block-start:calc\(0px - var\(--pm-space-1\)\)[^}]*margin-block-end:calc\(0px - var\(--pm-space-0\)\)/, '事件事实区上方收紧而下方保留自然间距');
